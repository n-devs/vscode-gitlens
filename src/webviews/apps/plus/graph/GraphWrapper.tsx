...
import {
  ...
  MenuLabel,
  MenuList,
} from "../../shared/components/menu/react";
...
const createIconElements = (): { [key: string]: ReactElement } => {
  ...
  iconList.forEach((iconKey) => {
    elementLibrary[iconKey] = createElement("span", {
      className: `graph-icon icon--${iconKey}`,
    });
  });
  miniIconList.forEach((iconKey) => {
    elementLibrary[iconKey] = createElement("span", {
      className: `graph-icon mini-icon icon--${iconKey}`,
    });
  });
...
export function GraphWrapper({
  ...
  const [includeOnlyRefsById, setIncludeOnlyRefsById] = useState(
    state.includeOnlyRefs
  );
  const [context, setContext] = useState(state.context);
  const [pagingHasMore, setPagingHasMore] = useState(
  ...
  const minimapData = useMemo(() => {
    ...
    const enabledMinimapMarkers: GraphMinimapMarkerTypes[] =
      graphConfig?.enabledMinimapMarkerTypes ?? [];
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        row.heads.forEach((h) => {
          ...
          if (
            enabledMinimapMarkers.includes(
              GraphMinimapMarkerTypes.LocalBranches
            ) ||
            (enabledMinimapMarkers.includes(GraphMinimapMarkerTypes.Head) &&
              h.isCurrentHead)
          ) {
      ...
      if (
        ...
        (enabledMinimapMarkers.includes(GraphMinimapMarkerTypes.Upstream) ||
          enabledMinimapMarkers.includes(
            GraphMinimapMarkerTypes.RemoteBranches
          ) ||
          enabledMinimapMarkers.includes(GraphMinimapMarkerTypes.LocalBranches))
      ) {
      ...
      if (
        row.type === "stash-node" &&
        enabledMinimapMarkers.includes(GraphMinimapMarkerTypes.Stashes)
      ) {
      ...
      if (
        row.tags?.length &&
        enabledMinimapMarkers.includes(GraphMinimapMarkerTypes.Tags)
      ) {
  ...
  const minimapSearchResults = useMemo(() => {
    if (
      !graphConfig?.minimap ||
      !graphConfig.enabledMinimapMarkerTypes?.includes(
    ...
    const searchResultsByDay = new Map<
      number,
      GraphMinimapSearchResultMarker
    >();
  ...
  const handleOnMinimapDaySelected = (
    e: CustomEvent<GraphMinimapDaySelectedEventDetail>
  ) => {
    let { sha } = e.detail;
    if (sha == null) {
  ...
  const searchPosition: number = useMemo(() => {
    ...
    if (searchIndex == null) {
      [searchIndex] = getClosestSearchResultIndex(
        searchResults,
        searchQuery,
        activeRow
      );
  ...
  const ensureSearchResultRow = async (
    id: string
  ...
  const handleSearchNavigation = async (
    e: CustomEvent<SearchNavigationEventDetail>
  ) => {
    ...
    if (direction === "first") {
      ...
      next = direction === "next";
      [searchIndex, id] = getClosestSearchResultIndex(
        results,
        searchQuery,
        activeRow,
        next
      ...
      if (searchIndex == -1) {
        if (next) {
          if (searchQuery != null && results?.paging?.hasMore) {
            ...
            let moreResults;
            try {
              moreResults = await onSearchPromise?.(searchQuery, {
                more: true,
              });
            } finally {
              setSearching(false);
            ...
            if (
              moreResults?.results != null &&
              !("error" in moreResults.results)
            ) {
  ...
  const handleExcludeTypeChange = (e: Event | FormEvent<HTMLElement>) => {
    ...
    if (
      !isLocalBranches &&
      !["remotes", "stashes", "tags", "mergeCommits"].includes(value)
    )
      return;
    ...
    if (
      (currentFilter == null && isChecked) ||
      (currentFilter != null && currentFilter !== isChecked)
    ) {
      setExcludeTypes({
        ...excludeTypes,
        [key]: isChecked,
  ...
  const handleOnColumnResized = (
    columnName: GraphColumnName,
    columnSettings: GraphColumnSetting
  ) => {
  ...
  const handleOnGraphColumnsReOrdered = (
    ...
    for (const [columnName, config] of Object.entries(
      columnsSettings as GraphColumnsConfig
    )) {
  ...
  const handleOnToggleRefsVisibilityClick = (
    _event: any,
  ...
  const handleSelectGraphRows = (rows: GraphRow[]) => {
    const active = rows[0];
    const activeKey =
      active != null ? `${active.sha}|${active.date}` : undefined;
    ...
    return (
      <span className="badge-container mr-loose">
        <span className="badge is-help">
          <span className={`repo-access${isPro ? " is-pro" : ""}`}>✨</span>{" "}
          {label}
          {subText && (
            <>
              &nbsp;&nbsp;
        ...
        content = (
          <>
            ...
            <p className="alert__message">
              You have {days < 1 ? "less than one day" : pluralize("day", days)}{" "}
              left in your 3-day GitLens Pro trial. Don't worry if you need more
              time, you can extend your trial for an additional free 7-days of
              the Commit Graph and other{" "}
              <a href="command:gitlens.plus.learn">GitLens+ features</a> on
              private repos.
            </p>
          </>
        );
      ...
      case SubscriptionState.FreePlusInTrial:
        ...
        content = (
          <>
            ...
            <p className="alert__message">
              You have {days < 1 ? "less than one day" : pluralize("day", days)}{" "}
              left in your GitLens Pro trial. Once your trial ends, you'll
              continue to have access to the Commit Graph and other{" "}
              <a href="command:gitlens.plus.learn">GitLens+ features</a> on
              local and public repos, while upgrading to GitLens Pro gives you
      ...
      case SubscriptionState.FreePreviewTrialExpired:
        ...
        content = (
          <>
            ...
            <p className="alert__message">
              Your free 3-day GitLens Pro trial has ended, extend your trial to
              get an additional free 7-days of the Commit Graph and other{" "}
              <a href="command:gitlens.plus.learn">GitLens+ features</a> on
              private repos.
            </p>
          </>
      ...
      case SubscriptionState.FreePlusTrialExpired:
        ...
        content = (
          <>
            ...
            <p className="alert__message">
              Your GitLens Pro trial has ended, please upgrade to GitLens Pro to
              continue to use the Commit Graph and other{" "}
              <a href="command:gitlens.plus.learn">GitLens+ features</a> on
              private repos.
            </p>
          </>
        ...
        content = (
          <>
            ...
            <p className="alert__message">
              Before you can use{" "}
              <a href="command:gitlens.plus.learn">GitLens+ features</a> on
              private repos, please verify your email address.
            </p>
          </>
        );
        actions = (
  ...
  const renderFetchAction = () => {
    ...
    if (branchState) {
      ...
      tooltip += ` ${branchState.upstream}${
        branchState.provider ? ` on ${branchState.provider}` : ""
      }`;
    }
  ...
  return (
    <>
      ...
      <header className="titlebar graph-app__header">
          ...
          <div className="titlebar__row">
            <div className="titlebar__group">
              ...
              <SearchBox
                ...
                valid={Boolean(
                  searchQuery?.query && searchQuery.query.length > 2
                )}
                ...
                onChange={(e) =>
                  handleSearchInput(e as CustomEvent<SearchQuery>)
                }
                onNavigate={(e) =>
                  handleSearchNavigation(
                    e as CustomEvent<SearchNavigationEventDetail>
                  )
                }
        ...
        <div
          className={`progress-container infinite${isLoading ? " active" : ""}`}
          role="progressbar"
        >
      ...
      {graphConfig?.minimap && (
        <GraphMinimap
          ...
          onSelected={(e) =>
            handleOnMinimapDaySelected(
              e as CustomEvent<GraphMinimapDaySelectedEventDetail>
            )
          }
        ></GraphMinimap>
      )}
      ...
      >
          ...
          <>
            <GraphContainer
              ...
              enabledScrollMarkerTypes={
                graphConfig?.enabledScrollMarkerTypes as
                  | GraphMarkerType[]
                  | undefined
              }
              ...
              highlightedShas={
                searchResults?.ids as GraphContainerProps["highlightedShas"]
              }
              ...
              onGraphMouseLeave={
                minimap.current ? handleOnGraphMouseLeave : undefined
              }
              onGraphRowHovered={
                minimap.current ? handleOnGraphRowHovered : undefined
              }
              ...
              onGraphVisibleRowsChanged={
                minimap.current ? handleOnGraphVisibleRowsChanged : undefined
              }
        ...
        <button
          ...
          data-vscode-context={
            context?.header ||
            JSON.stringify({ webviewItem: "gitlens:graph:columns" })
          }
...
function formatCommitDateTime(
  ...
  switch (source) {
    ...
    default:
      return style === DateStyle.Relative
        ? fromNow(date)
...
function getClosestSearchResultIndex(
  ...
  if (index == null) {
    const activeDate =
      activeInfo?.date != null ? activeInfo.date + (next ? 1 : -1) : undefined;
    ...
    for ([id, { date, i }] of Object.entries(results.ids)) {
      if (next) {
        ...
        date > activeDate &&
        (nearestDate == null || date <= nearestDate)
      ) {
        nearestId = id;
        nearestDate = date;
        nearestIndex = i;
    ...
    index =
      nearestIndex == null ? results.count - 1 : nearestIndex + (next ? -1 : 1);
  }
...
function getSearchResultIdByIndex(
  results: GraphSearchResults,
  index: number
): string | undefined {
